...
export function LoginScreen() {
  ...
  const isLoading = state.isLoading || isSubmitting;
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-red-50 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center p-4">
      <div className="w-full max-w-md bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">{t('auth.welcomeBack')}</h1>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="relative">
            <Input label={t('auth.password')} type={showPassword ? 'text' : 'password'} {...register('password')} placeholder={t('auth.enterPassword')} required error={errors.password?.message} disabled={isLoading} className="pl-10 pr-10" />
          ...
          <div className="text-center"><span className="text-gray-600 dark:text-gray-400">{t('auth.dontHaveAccount')}{' '}</span><Link to="/signup" className="text-orange-500 hover:text-orange-600 font-medium">{t('auth.signup')}</Link></div>
        </form>
      </div>
    </div>
  );
